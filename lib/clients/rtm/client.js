...
var parseRTMMessage = require('./helpers').parseRTMMessage;
 ...
 * @constructor
 */
var RTMClient = function (webClient, socketFn, opts) {
...
RTMClient.prototype = create(BaseAPIClient.prototype, {
...
RTMClient.prototype.handleWsMessage = function (message) {
    ...
    try {
        message = parseRTMMessage(message);
    } catch (err) {
    ...
    if (messageType === RTM_EVENTS.MESSAGE) {
        messageType = makeMessageEventWithSubtype(message.subtype, '::');
    }
    ...
    if (messageType === 'pong') {
        ...
        if (handler) {
            ...
            if (handler.length === 4) {
                var updatedMsg = handler(this.activeUserId, this.activeTeamId, this.dataStore, message);
            } else {
                var updatedMsg = handler(this.dataStore, message);
            }
        }

        this.emit('raw::' + messageType, message);
